...
public class LiveRoomActivity extends BaseActivity implements AGEventHandler {
    ...
    protected void initUIandEvent() {
        ...
        ImageView button1 = (ImageView) findViewById(R.id.btn_1);// 切换主播和观众
        ImageView button2 = (ImageView) findViewById(R.id.btn_2);
        ImageView button3 = (ImageView) findViewById(R.id.btn_3);

        if (isBroadcaster(cRole)) {
            if (mSource == null){
                mSource = new AgoraTextureCamera(this, 640, 480);
                mSharedContext = ((AgoraTextureCamera) mSource).getEglContext();
                worker().setVideoSource(mSource);
            }
            ...
            worker().updateRender(render);

            mUidsList.put(0, render); // get first surface view
    ...
    private void doRenderRemoteUi(final int uid) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                AgoraSurfaceView render = new AgoraSurfaceView(LiveRoomActivity.this);
                render.setZOrderOnTop(true);
                render.setZOrderMediaOverlay(true);
                mUidsList.put(uid, render);

                if (config().mUid == uid) {
                    if (mSource == null){
                        mSource = new AgoraTextureCamera(LiveRoomActivity.this, 640, 480);
                        mSharedContext = ((AgoraTextureCamera) mSource).getEglContext();
                        worker().setVideoSource(mSource);
                    }
                    ...
                    worker().addRemoteRender(uid, render);
                }
